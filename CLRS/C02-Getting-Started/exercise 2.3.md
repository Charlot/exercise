...
![recurrence](../attach/clrs-e-2-3-4.png)
...
![recursion tree](../attach/clrs-e-2-3-p-1.png)
...
为了Θ(nk+nlg(n/k)) 的增长小于等于 Θ(nlgn)，其中Θ(nk)或者Θ(nlg(n/k))的增长都小于 Θ(nlgn)。

必然Θ(nlg(n/k))的增长小于 Θ(nlgn)，那么需要Θ(nk)的增长小于 Θ(nlgn)，所有k最大为lgn。

> d

取k为子列表的长度最大值，使插入排序比归并排序快；
c1k^2≤c2klgk
...
b. 为第2~4行的for循环精确地说明一个循环不变式(oop invariant)，并证明该循环不变式成立。你的证明应该使用本章中给出的循环不变式证明的结构。
...
**初始化 Initialization**:
当j=n时，子数组A[n]只包含一个元素，即A的最后一个元素，是子数组A[n]的最小值，循环不变式成立；

**保持 Maintenance**：
每次循环时，子数组为A[j-1..n]，会把最小值向前放，使A[j-1]为最小值，循环不变式成立；

**终止 Termination**：
循环结束时，j=i，子数组为A[i..n]，最小值为A[i]，循环不变式成立；

> c

**循环不变式**:
A的子数组A[1..i-1]是A中i-1个最小值且已排序按从小到大排序；

**初始化 Initialization**:
循环开始时，i=1，子数组为空，循环不变式成立；

**保持 Maintenance**：
有b可知，每次循环A[i-1]为A[i..n]中最小值，循环不变式成立
 
**终止 Termination**：
循环结束时，i=n，A[n-1]为A[n..n]中最小值，即A[n]为A[1..n]中最大值，A[1..n]为已排序的数组，循环不变式成立；

> d

冒泡排序的最坏情况是Θ(n^2)，最坏情况与插入排序一样
...
![霍纳规则的准确性-中文](../attach/clrs-e-2-3-p-3-1.png)

![correction of horner's rule](../attach/clrs-e-2-3-p-3.png)
...
**初始化(Initialization)** ：
当第一次循环开始前，y=0, 循环不变式成立

**保持(Maintenance)**：
在循环过程中，当前i项值为：

$$ \begin{aligned}  y & = a_i + x * (\sum_{k = 0}^{n - (i + 1)} a_{k + i + 1} x^k) 
...
c. 插入排序的运行时间与输人数组中逆序对的数量之间是什么关系？证明你的回答。
